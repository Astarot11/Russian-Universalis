...
VertexShader = 
{
	...
	[[
		VS_OUTPUT_BORDER main( const VS_INPUT_BORDER VertexIn )
		{
			...
			pos.y = vClampHeight * WATER_HEIGHT + ( 1.0f - vClampHeight ) * pos.y +.1; // clipping
...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT_BORDER Input ) : PDX_COLOR
		{
			...
			clip( 0.99f - TI );
			float4 vColor = tex2D( BorderDiffuse, float2( Input.uv.y * BORDER_TILE, Input.uv.x ) )*1.2f; //diffuse border color
