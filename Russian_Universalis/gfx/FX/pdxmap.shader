...
PixelShader = 
{
	...
	[[
		...
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
			...
			float lod = clamp( trunc( mipmapLevel( vTileRepeat ) - 0.5f ), 0.0f, 2.0f );
		...
		#endif
		{
            ...
            // Handle snow, if you don't want snow, add comments to line 567 and 568
            ...
            vOut = float3(  min( ( vColorMapSample_blended.r * vBlend_saturation.x * vTerrainDiffuseSample_green + vTerrainDiffuseSample.r * vBlend_saturation.y ) * vBlend_flux.r , 2.0f ) ,
                            min( ( vColorMapSample_blended.g * vBlend_saturation.x * vTerrainDiffuseSample_green + vTerrainDiffuseSample.r * vBlend_saturation.y ) * vBlend_flux.g , 1.0f ) ,
                            min( ( vColorMapSample_blended.b * vBlend_saturation.x * vTerrainDiffuseSample_green + vTerrainDiffuseSample.r * vBlend_saturation.y ) * vBlend_flux.b , 1.0f ) );
			//Dynamic test
            ...
            float3 vOut2 = CalculateMapLighting( vOut, vHeightNormalSample );
